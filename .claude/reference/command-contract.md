# RAPTOR command-specific reference

This is the shared RAPTOR agent contract despite the legacy filename.
...
2. On a single line, output "Quick commands:" followed by `agentic`, `scan`, `fuzz`, `web`, and `commands` for the full list. Prefix them with `$` when `RAPTOR_AGENT=codex`; otherwise prefix them with `/`. Do not explain what they do.
...
Command files use `/name` as the provider-neutral notation. In operator-facing
responses, examples, follow-up choices, and usage hints, render commands as
`$name` when `RAPTOR_AGENT=codex`; render `/name` on every other host. This is
display-only: keep canonical file paths, dispatch lookup, and mechanical CLI
commands unchanged.
...
**Note:** `/agentic` runs scan → dedup → prep → analysis (with validation methodology). Use `--sequential` to bypass parallel orchestration. Use `--understand` to pre-map the codebase before scanning, `--validate` to run the full validation pipeline on exploitable findings afterwards, and `--gap-audit` to run the /audit orchestrator over the coverage residual (functions no phase reviewed; uses the selected host CLI in a host-bound session, or the configured provider in a standalone run; NOT `--audit`, which is the sandbox audit mode). All three flags are opt-in. Multi-model: `--model` is repeatable — multiple models each independently analyse every finding, then results are correlated; `--consensus`, `--judge`, and `--aggregate` add optional review/synthesis models.
...
After a completed run, use the output directory reported by the lifecycle as
the authoritative result location. Read and summarize the produced report,
including finding locations, evidence, exploit or PoC artifacts, patch
artifacts, validation failures, skipped stages, and unresolved findings. Do not
claim that an artifact exists unless it was emitted. Offer the next relevant
command or deeper review. Applying a generated patch remains a dangerous
operation and requires operator confirmation.
...
Python orchestrates execution. The selected agent shows results concisely.
