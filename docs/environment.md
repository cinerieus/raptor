...
|----------|---------|---------|
| `RAPTOR_AGENT` | `auto` | Interactive host selected by `bin/raptor`: `claude`, `codex`, or `opencode`. An explicit `--agent` flag wins. Analysis selection remains independent: explicit analysis flags, `models.json`, and API/environment providers take precedence, then the selected CLI subscription is the keyless fallback. Auto-detection preserves Claude-first compatibility, then checks Codex and OpenCode. |
| `RAPTOR_AGENT_MODEL` | selected host default | Model pinned by launcher `--model` for subscription analysis children. When unset, the selected CLI resolves its normal operator-owned model and provider configuration. Usually set by `bin/raptor`, not by hand. |
...
| `RAPTOR_ENV_FLOOR_MIB` | unset (built-in floors) | Byte-floor override for the audit environment guard (`core.audit.environment`): the value in MiB becomes the preflight refuse floor and the watchdog pause floor; the preflight warn floor scales at 8× and the watchdog resume floor at 2× (default ratios preserved). `0` disables the byte floors entirely; inode floors are unaffected. Sanctioned way to run on a deliberately tight filesystem (container tmpfs, fault-injection rig) that the default 64 MiB refuse floor would reject at start or pause permanently. Non-numeric or negative values are ignored with a warning (a typo must never silently disable the guard). The systemic-fault circuit breaker has its own per-run flag, `raptor-audit run --no-environment-breaker`. |
...
| `RAPTOR_NO_LAUNCHER_HARDENING` | unset | Any non-empty value skips the `bin/raptor` exec-boundary hardening block entirely: the soft core-dump cap, the umask floor (current \| `022`), the PATH scrub (empty / relative / world-writable entries), the world-writable-ancestor warning on resolved coding agents, and the per-session TMPDIR (creation and stale-sibling sweep). Single opt-out for environments that legitimately violate one of the checks. Fail-closed: unset means hardening runs. |
...
| `RAPTOR_SANDBOX_TIMEOUT` | `1800` | Child wall-clock limit (seconds, positive integer) for `libexec/raptor-run-sandboxed` — the untrusted build/PoC wrapper. Raise per run for large/ASAN builds that legitimately exceed 30 minutes; a malformed value refuses loudly (exit 1), never falls back silently. |
...
| `RAPTOR_AGENT_CLI_MAX_CALLS` | `100` | Hard process-wide inference-call ceiling for Codex/OpenCode subscription transports. Must be a positive integer. This bounds subscription usage because those CLIs do not expose a monetary cost suitable for `--max-cost`. |
| `RAPTOR_AGENT_CLI_PROXY_HOSTS` | unset | Comma-separated additional HTTPS hostnames allowed through the Codex/OpenCode inference sandbox, for custom OpenCode providers or corporate gateways. Entries are validated as hostnames; schemes, ports, paths, and wildcards are rejected. |
...
| `RAPTOR_TEST_EMULATE_PLATFORM` | unset | Exactly `darwin` runs the pytest session with `sys.platform` patched to `darwin` before collection, pytest's basetemp relocated under a darwin-shaped `/private/var/folders` path, and every `darwin_native`/`linux_native`-marked test deselected (emulation never stacks on real kernel behaviour; the markers declare a real-kernel binding). Catches Linux-authored assertions against platform-resolved values in the Linux CI lane. Any other value is a hard error. In-process only — subprocess children observe the real platform. |
