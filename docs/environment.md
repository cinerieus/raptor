...
|----------|---------|---------|
| `RAPTOR_AGENT` | `auto` | Interactive host and inference transport used by `bin/raptor`: `claude`, `codex`, or `opencode`. An explicit `--agent` flag wins. The selected CLI subscription is authoritative, so ambient model API keys cannot override it. Auto-detection preserves Claude-first compatibility, then checks Codex and OpenCode. |
