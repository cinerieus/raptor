...
def test_pr_comment_cli_flag_via_main(tmp_path: Path, capsys) -> None:
    ...
    # --offline keeps the test hermetic: we're only checking that the
    # flag is parsed + dispatched, and any future unconditional probe
    # must not turn this into a network-dependent test that hangs
    # through retry backoff on CI.
