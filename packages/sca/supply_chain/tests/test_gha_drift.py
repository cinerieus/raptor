...
""")
    ...
    # The host is the ACTION dep, key-equal with the row the
    # inline-installs workflow parser emits for the same line — the
    # composite chokepoint's GHA+SENTINEL pair depends on that.
    assert f.dependency.ecosystem == "GitHub Actions"
    assert f.dependency.name == "actions/checkout"
    assert f.dependency.version == "v4"
    assert f.dependency.purl == "pkg:githubactions/actions/checkout@v4"
