...
_PARSE_CACHE: dict[Path, VersionCatalog | None] = {}


def parse_libs_versions_toml(path: Path) -> VersionCatalog | None:
    ...
    # and only then used as the cache key.
...
def find_default_catalog(repo: Path) -> Path | None:
